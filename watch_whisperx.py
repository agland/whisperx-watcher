...
# Config flags (from environment / config.env)
...
if __name__ == "__main__":
    ...
    logger.info(f"RECORDINGS_DIR={RECORDINGS_DIR}")
    logger.info(f"SCAN_SUBDIRS={SCAN_SUBDIRS}")

    # Process any existing files on startup
    if SCAN_SUBDIRS:
        logger.info(f"Initial scan (recursive) under: {RECORDINGS_DIR}")
        for p in sorted(RECORDINGS_DIR.rglob("*")):
            if p.is_file() and is_audio(p):
                logger.info(f"Initial scan: processing {p}")
                process_file(p)
    else:
        logger.info(f"Initial scan (flat) in: {RECORDINGS_DIR}")
    ...
    observer = Observer()
    observer.schedule(Handler(), str(RECORDINGS_DIR), recursive=SCAN_SUBDIRS)
    observer.start()
